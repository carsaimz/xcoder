...
const originalXCoder = globalThis.xcoder;
...
describe("actionStack app exit", () => {
	beforeEach(() => {
		...
		globalThis.xcoder = { exitAppMessage: "Close XCoder?" };
	...
	afterAll(() => {
		actionStack.onCloseApp = undefined;
		globalThis.xcoder = originalXCoder;
	...
	it("exits without showing an interstitial after confirmation", async () => {
		...
		expect(mocks.confirm).toHaveBeenCalledWith("WARNING", "Close XCoder?");
