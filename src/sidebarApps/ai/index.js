...
import { activeBot, listBots, setActiveBot } from "lib/ai/bots";
import {
        generateImage,
        parseImageArgs,
        saveGeneratedImage,
...
function initApp(el) {
        ...
        return () => {
                try {
                        Sidebar?.off?.("show", onSelected);
                } catch {
                        /* older builds only expose on() */
                }
                container = null;
        ...
        $toggles = (
                <div className="ai-toggles">
                        <button
                                type="button"
                                className="ai-toggle"
                                data-toggle="thinking"
                                onclick={() => setQuickToggle("thinking")}
                        >
                        ...
                        >
                                <span className="icon smart_toy" />
                                <span className="ai-toggle-label" data-role="bot-label">
                                        {strings["ai bot pill"] || "Bot"}
                                </span>
                        </button>
                </div>
        );
...
/** Refreshes the bot pill label/icon from the active bot (or generic). */
function updateBotPill() {
        if (!$toggles) return;
        const $label = $toggles.querySelector('[data-role="bot-label"]');
        if (!$label) return;
        const bot = activeBot();
        $label.textContent = bot
                ? `${bot.icon} ${bot.name}`
                : strings["ai bot pill"] || "Bot";
        const $pill = $toggles.querySelector('[data-toggle="bot"]');
        $pill?.classList.toggle("on", Boolean(bot));
        if ($pill) {
                $pill.title = bot
                        ? `${strings["ai bot active title"] || "Bot ativo"}: ${bot.name}`
                        : strings["ai bot pick title"] || "Escolher bot";
        }
}

/** Bot picker — one-tap personas; manage in AI settings > Bots. */
async function openBotPicker() {
        const bots = listBots();
        const items = [
                {
                        text: strings["ai bot none"] || "Nenhum (bot desligado)",
                        value: "__none__",
                },
                ...bots.map((bot) => ({
        ...
        const choice = await select(strings["ai bot pick"] || "Escolher bot", items);
        if (!choice) return;
        if (choice === "__none__") {
                await setActiveBot("");
                updateBotPill();
        ...
        const choice = await modelPicker({
                ...
                actions: [
                        ...
                        },
                        {
                                value: "__local__",
                                text:
                                        strings["ai models local"] ||
                                        "Baixar modelos locais (offline)",
                        },
                ],
        });
        if (!choice) return;
