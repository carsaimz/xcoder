...
async function loadMorePlugins() {
	try {
		...
		const newPlugins = all.slice(start, start + LIMIT);
...
async function runSearch(query) {
	...
	try {
		...
		const plugins = await pluginRegistry.search(query);
...
async function loadExplore() {
	...
	try {
		...
		const all = await pluginRegistry.list();
		const plugins = all.slice(0, LIMIT);
...
async function getFilteredPlugins(filterState) {
	...
	// XCoder: registry is local — filter the full local list at once.
...
function ListItem({
	...
	$el.onclick = async (event) => {
		...
		if (installPluginBtn) {
			try {
				const regPlugin = await pluginRegistry.get(id);
				const { default: installPlugin } = await import("lib/installPlugin");
				await installPlugin(
					regPlugin?.url || regPlugin?.source || id,
					regPlugin?.name,
				);
	...
	switch (action) {
		...
		case strings.enable || "Enable":
			...
			if (choice === "restart_app") {
				...
				if (enabled) {
					window.xcoder.unmountPlugin(id);
