...
public class WebViewInstance {
  ...
  private static final String BRIDGE_JS =
    "(function(){" +
    "if(window.webview&&window.webview.__xcoderBridge){return;}" +
    "var callbacks=[];" +
    "window.webview={" +
    "__xcoderBridge:true," +
    ...
    "window.XCoderWebViewNative.postMessage(String(data));" +
    "}," +
  ...
  void createWebView(Activity activity) {
    ...
    webView.addJavascriptInterface(new JsBridge(), "XCoderWebViewNative");
    ...
    runOnUiThread(new Runnable() {
      ...
      public void run() {
        if (webView != null) {
          ...
          webView.removeJavascriptInterface("XCoderWebViewNative");
