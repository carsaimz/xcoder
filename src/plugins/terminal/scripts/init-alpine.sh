...
if [ "$INSTALLING" = true ]; then
    ...
    if [ ! -e "$PREFIX/alpine/etc/xcoder_motd" ]; then
        cat <<EOF > "$PREFIX/alpine/etc/xcoder_motd"
Welcome to Alpine Linux in XCoder!
...
__xcoder_existing_debug_trap="$(trap -p DEBUG 2>/dev/null)"
if [[ -n "${__xcoder_existing_debug_trap}" ]]; then
    __xcoder_existing_cmd="$(printf "%s" "${__xcoder_existing_debug_trap}" | sed -E "s/.*'((.*)?)'.*/\1/")"
else
    __xcoder_existing_cmd=""
fi

# Only add our handler if it's not already present
if [[ "${__xcoder_existing_cmd}" != *"_xcoder_preexec"* ]]; then
    if [[ -n "${__xcoder_existing_cmd}" ]]; then
        trap "${__xcoder_existing_cmd}; _xcoder_preexec" DEBUG
    else
        trap '_xcoder_preexec' DEBUG
    fi
fi
unset __xcoder_existing_debug_trap __xcoder_existing_cmd
