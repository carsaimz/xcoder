...
const menuJava = path.resolve(
  __dirname,
  "../../../platforms/android/app/src/main/java/com/xcoder/browser/Menu.java"
);
const docProvider = path.resolve(
  __dirname,
  "../../../platforms/android/app/src/main/java/com/carsaimz/xcoder/rk/exec/terminal/AlpineDocumentProvider.java"
);
...
function replaceImport(filePath, appName) {
  ...
  const updated = data.replace(
    /(import\s+com\.xcoder\.)(xcoder|xcoderfree)(\.R;)/,
    `$1${appName}$3`
  );
...
try {
  ...
  const msg = `==== Changed package to com.xcoder.${appName} ====`;
