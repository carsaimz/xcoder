...
async function loadApp() {
	...
	if (Array.isArray(files) && files.length) {
		...
		// Restore the tab navigation history (back/forward) from the
		// previous session BEFORE the first save-state below — that
		// save would otherwise overwrite the stored history with an
		// empty one. Ids whose files are gone are skipped by the
		// editorManager.
		try {
			const savedHistory = helpers.parseJSON(localStorage.tabHistory);
			if (savedHistory && editorManager.restoreEditorHistory(savedHistory)) {
				window.log("info", "Tab history restored");
			}
		} catch (error) {
			window.log("error", error);
		}
