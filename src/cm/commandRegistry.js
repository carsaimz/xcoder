...
import {
        ...
        findAllReferences as xcoderFindAllReferences,
        findAllReferencesInTab as xcoderFindAllReferencesInTab,
...
function registerCoreCommands() {
        ...
        addCommand({
                ...
                requiresView: false,
                run() {
                        xcoder.exec("move-tab-to-new-pane");
                        return true;
                },
        });
        addCommand({
        ...
        addCommand({
                ...
                requiresView: false,
                run() {
                        xcoder.exec("focus-previous-pane");
                        return true;
                },
        });
        addCommand({
        ...
        addCommand({
                ...
                requiresView: false,
                run() {
                        xcoder.exec("change-editor-theme");
                        return true;
                },
        });
        addCommand({
                ...
                requiresView: false,
                run() {
                        xcoder.exec("new-terminal");
                        return true;
                },
        });
        addCommand({
                name: "xcoder:showWelcome",
...
function registerLspCommands() {
        ...
        addCommand({
                ...
                readOnly: false,
                requiresView: true,
                run: runLspCommand(xcoderRenameSymbol),
        });
        addCommand({
        ...
        addCommand({
                ...
                async run(view) {
                        ...
                        return xcoderFindAllReferences(resolvedView);
                },
        });
        addCommand({
                ...
                requiresView: false,
                run() {
                        return xcoderCloseReferencesPanel();
                },
        });
        addCommand({
                ...
                async run(view) {
                        ...
                        return xcoderFindAllReferencesInTab(resolvedView);
                },
        });
        addCommand({
...
async function openInAppBrowserCommand() {
        ...
        if (url) xcoder.exec("open-inapp-browser", url);
        return true;
}
                ...
                if (await bindingsFile.exists()) {
                        ...
                        Object.keys(keyBindings).forEach((key) => {
                                ...
                                if (!(key in bindings)) {
                                        ...
                                        stored.key == null &&
                                        keyBindings[key]?.key != null
                                ) {
                                        // A default shortcut shipped for an entry the user never
                                        // re-bound (stored key is still the original null) —
                                        // adopt it. Deliberately cleared bindings are rare and
                                        // can simply be cleared again in settings.
                                        bindings[key] = keyBindings[key];
                                        updated = true;
                                }
                        });
