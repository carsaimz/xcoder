...
import XCoderWorkspace from "./workspace";
...
interface ExtendedLSPClient extends LSPClient {
  __xcoderServerId?: string;
  __xcoderLoggedInfo?: boolean;
}
  ...
  async #ensureClient(
    ...
    if (this.#clients.has(key)) {
      ...
      if (useWsFolders && normalizedRootUri) {
        const workspace = existing.client.workspace as XCoderWorkspace | null;
    ...
    if (this.#pendingClients.has(key)) {
      ...
      if (useWsFolders && normalizedRootUri) {
        const workspace = pending.client.workspace as XCoderWorkspace | null;
        if (workspace && !workspace.hasWorkspaceFolder(normalizedRootUri)) {
          workspace.addWorkspaceFolder(normalizedRootUri);
        }
      }
    ...
    try {
      ...
      if (useWsFolders && normalizedRootUri) {
        const workspace = state.client.workspace as XCoderWorkspace | null;
        if (workspace && !workspace.hasWorkspaceFolder(normalizedRootUri)) {
          workspace.addWorkspaceFolder(normalizedRootUri);
        }
      }
  ...
  async #initializeClient(
    ...
    if (!clientConfig.workspace) {
      clientConfig.workspace = (client: LSPClient) =>
        new XCoderWorkspace(client, workspaceOptions);
    }
    ...
    try {
      ...
      client.__xcoderServerId = server.id;
      connectClient(
        client,
      ...
      if (!client.__xcoderLoggedInfo) {
        ...
        client.__xcoderLoggedInfo = true;
      }
    } catch (error) {
      addLspLog(
        server.id,
    ...
    const detach = (uri: string, view?: EditorView): void => {
      ...
      if (!view || !existing.size) {
        ...
        try {
          ...
          (client.workspace as XCoderWorkspace)?.closeFile?.(actualUri);
        } catch (error) {
