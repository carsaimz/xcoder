...
class XCoderWorkspaceFile implements WorkspaceFile {
	uri: string;
	languageId: string;
	version: number;
	doc: Text;
...
export default class XCoderWorkspace extends Workspace {
	files: XCoderWorkspaceFile[];
	options: WorkspaceOptions;

	#fileMap: Map<string, XCoderWorkspaceFile>;
	...
	#getFileEntry(uri: string): XCoderWorkspaceFile | null {
		return this.#fileMap.get(uri) ?? null;
	}

	#removeFileEntry(file: XCoderWorkspaceFile): void {
	...
	getFile(uri: string): XCoderWorkspaceFile | null {
		return this.#getFileEntry(uri);
	}

	requestFile(uri: string): Promise<XCoderWorkspaceFile | null> {
