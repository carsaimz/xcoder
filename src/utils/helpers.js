...
export default {
	...
	/**
	 * Whether the ads system may show ads right now (user toggle +
	 * site config). House ads come from the community site; native
	 * providers plug in via lib/ads.registerNativeProvider().
	 * @returns {boolean}
	 */
	canShowAds() {
		// user toggle; the full cap check lives in lib/ads.js
		return settings.value.adsEnabled !== false;
	},
	async showInterstitialIfReady() {
		try {
			const ads = await import("lib/ads");
			return ads.maybeShowInterstitial("showInterstitialIfReady");
		} catch {
			return false;
		}
	},
	/**
	 * House ads from the community site with hard frequency caps (no
	 * AdMob). Shows an interstitial only on natural break points and
	 * never more often than the site-configured caps; no-ops when ads
	 * are disabled, offline or within the grace period.
	 */
	showAd() {
		import("lib/ads")
			.then(({ maybeShowInterstitial }) =>
				maybeShowInterstitial("helpers.showAd"),
			)
			.catch(() => {});
	},
	...
	/**
	 * XCoder runs fully offline. Always returns true so local-only
	 * features (search, install from URL) stay available.
	 */
	...
	async createFileStructure(uri, pathString, isFile = true) {
		...
		const isXCoderTerminalPublicSafUri = currentUri.includes(
			"com.carsaimz.xcoder.documents",
		);
		const [, treeSegment = ""] = currentUri.split("/tree/");
		const terminalBasePath = isXCoderTerminalPublicSafUri
			? decodeURIComponent(treeSegment.split("::")[0] || "")
			: "";
		const getTargetUri = (baseUri, name, index) => {
			if (
				!(isExternalStorageUri || isTermuxUri || isXCoderTerminalPublicSafUri)
			) {
			...
			} else if (isXCoderTerminalPublicSafUri) {
				if (!isSpecialCase && index === 0) {
