...
const isXCoderTerminalPublicSafUri = (value = "") =>
	value.startsWith("content://com.carsaimz.xcoder.documents/tree/");
const isTerminalSafUri = (value = "") =>
	isTermuxSafUri(value) || isXCoderTerminalPublicSafUri(value);
...
const isTerminalAccessiblePath = (url = "") => {
	if (isXCoderTerminalPublicSafUri(url)) return true;
...
const convertToProotPath = (url = "") => {
	const { alpineRoot, publicDir } = getTerminalPaths();
	if (isXCoderTerminalPublicSafUri(url)) {
		try {
...
const XCODER_PLUGIN_MANIFEST_FILE = "plugin.json";
/**
...
async function handleContextmenu(type, url, name, $target) {
	...
	if (helpers.isFile(type)) {
		...
		if (
			...
			(await fsOperation(
				Url.dirname(url) + XCODER_PLUGIN_MANIFEST_FILE,
			).exists())
		) {
