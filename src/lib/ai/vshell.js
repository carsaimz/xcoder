...
import { cmdGh, cmdHttp, cmdJq, cmdPkg, cmdPython, installed } from "./pkg";
...
/**
 * Runs a package-provided command, or hints how to install it.
 * @param {string} name package/command name
 * @param {() => Promise<{output: string, error?: boolean}>} run
 * @returns {Promise<{output: string, error?: boolean}>}
 */
async function pkgCommand(name, run) {
	if (!installed().includes(name)) {
		return {
			output: `${name}: command not found (install it with 'pkg install ${name}')`,
			error: true,
		};
	}
	return await run();
...
/**
 * Editor-backed nano: opens the file in XCoder (creates an empty one
 * when missing, like nano does on first save).
 * @param {string[]} args
 * @returns {Promise<{output: string, error?: boolean}>}
 */
async function cmdNano(args) {
	...
	if (!target) {
		return { output: "nano: missing file operand", error: true };
	}
	...
	await openFile(url);
	return { output: `opened ${nameOf(url)} in the editor` };
}

/**
 * Opens a file in the XCoder editor (bridge shell -> editor).
 * @param {string[]} args
 * @returns {Promise<{output: string, error?: boolean}>}
 */
async function cmdOpen(args) {
	...
	const url = resolvePath(target);
	if (!(await exists(url))) {
		return { output: `open: ${target}: no such file`, error: true };
	}
	const { default: openFile } = await import("lib/openFile");
	await openFile(url);
	return { output: `opened ${nameOf(url)}` };
}
	...
	try {
		switch (cmd) {
			...
			case "wc": {
				...
				if (content == null)
					return { output: "wc: file not found", error: true };
			...
			// --- Termux-like packages (local catalog, see lib/ai/pkg.js)
			case "pkg":
				return await cmdPkg(args, { exec });
			case "jq":
				return await pkgCommand("jq", () =>
					cmdJq(args, { readText, resolvePath }),
				);
			case "http":
				return await pkgCommand("http", () => cmdHttp(args));
			case "gh":
				return await pkgCommand("gh", () => cmdGh());
			case "python3":
			case "python":
				return await pkgCommand("python3", () => cmdPython());
			case "nano":
				return await pkgCommand("nano", () => cmdNano(args));
			case "help":
				return {
					output: [
						"available commands:",
						"  pwd, cd, ls, cat, head, echo, wc",
						"  mkdir, touch, rm, cp, mv, find, grep, open",
						"  pkg list | pkg search <term> | pkg install <name> | pkg uninstall <name>",
						"  installable: jq, http, nano, gh, python3  (e.g. 'pkg install jq')",
...
async function cmdCopy(args, move) {
	...
	if (files.length < 2) {
		return {
			output: `${move ? "mv" : "cp"}: missing destination`,
			error: true,
		};
	}
	...
	if (!(await exists(from))) {
		return {
			output: `${move ? "mv" : "cp"}: ${files[0]}: not found`,
			error: true,
		};
	}
...
async function cmdGrep(args) {
	...
	await walk(base, 3, async (url, isDir) => {
		...
		content.split("\n").forEach((line, index) => {
			if (regex.test(line)) {
				results.push(
					`${displayPath(url)}:${index + 1}: ${truncate(line, 200)}`,
				);
			}
		});
	});
...
async function cmdVcs(args) {
	...
	switch (sub) {
		...
		case "commit": {
			...
			await saveDb(db);
			return {
				output: `[${id}] ${message} (${Object.keys(files).length} files)`,
			};
		}
		case "log": {
			const db = await loadDb();
		...
		case "restore": {
			...
			if (!commit)
				return { output: `vcs: commit ${id} not found`, error: true };
			let restored = 0;
