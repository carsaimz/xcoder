...
export const TOOLS = [
        {
                ...
                async run({ path }) {
                        ...
                        return list
                                .map((item) => `${item.isDirectory ? "dir " : "file"} ${Url.basename(item.url)}`)
                                .join("\n");
                },
        },
        {
        ...
        },
        {
                ...
                async run({ path, content }) {
                        ...
                        await fsOperation(dir).createFile(Url.basename(url), String(content ?? ""));
        ...
        },
        {
                ...
                parameters: {
                        ...
                        properties: {
                                ...
                                path: { type: "string", description: "optional base path ('.' default)" },
                        },
                        required: ["query", "mode"],
                },
                async run({ query, mode, path }) {
                        const result =
                                mode === "find"
                                        ? await shellExec(`find ${path || "."} -name ${JSON.stringify(query)}`)
        ...
        },
        {
                name: "run_command",
                description:
                        "Run a virtual shell command (ls, cd, cat, mkdir, rm, cp, mv, find, grep, wc, vcs...). 'vcs commit <msg>' snapshots the workspace; 'vcs restore <id>' restores a snapshot.",
                danger: "exec",
                parameters: {
                        type: "object",
        ...
        },
        {
                ...
                async run() {
                        const { getEditorContext, formatEditorContext } =
                                await import("./editorBridge");
                        return formatEditorContext(
                                getEditorContext(window.editorManager),
                        );
                },
        },
        {
...
export const TOOL_MAP = Object.fromEntries(TOOLS.map((tool) => [tool.name, tool]));
...
export function toolSchemas(allowlist) {
        return TOOLS.filter((tool) => !allowlist || allowlist.includes(tool.name)).map(
                (tool) => ({
                        type: "function",
                        function: {
                                name: tool.name,
