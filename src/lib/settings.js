...
import lang, { detectDefaultLanguage } from "./lang";
...
class Settings {
	...
	constructor() {
		this.#defaultSettings = {
			...
			aiMode: "agent",
			aiSubagents: true,
			aiShowThinking: true, // "Pensar" pill — must exist in defaults or settings.update() drops it
			aiWebTools: true, // "Buscar" pill — must exist in defaults or settings.update() drops it
			aiSystemPrompt: "",
			aiProviderPrefs: {}, // per-provider overrides (key/url/tokens/autonomy/status)
			aiActiveBot: "", // active AI bot persona ("" = none)
			aiUserBots: [], // user-created AI bots (lib/ai/bots.js)
			gitRemoteUrl: "",
			ghRepo: "",
			ghToken: "",
			ghBranch: "main",
