...
class XCoder {
	...
	constructor() {
		...
		const deprecatedFileList = (...args) => {
			if (!didWarnAboutFileList) {
				...
				console.warn(
					'xcoder.require("fileList") is deprecated. Use the asynchronous "fileIndex" API. fileList now contains only non-native storage providers.',
				);
			}
			return files(...args);
		};
	...
	/**
	 * Installs an XCoder plugin
	 * @param {string} pluginId plugin id, local zip path or remote URL
		...
		return new Promise((resolve, reject) => {
			fsOperation(Url.join(PLUGIN_DIR, pluginId))
				...
				.then(async (isPluginExists) => {
					...
					const { default: installPlugin } = await import(
						/* webpackChunkName: "installPlugin" */ "lib/installPlugin"
					);
					await installPlugin(pluginId, installerPluginName);
					resolve();
				})
				.catch((error) => {
					reject(error);
