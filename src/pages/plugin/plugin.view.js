...
async function Buttons(props) {
	...
	if (
		...
		return (
			<div className="error">
				...
				<a href={`${config.GITHUB_URL}/issues`} className="text">
	...
	// XCoder: purchases/login are removed — paid plugins are not supported.
...
function LegacyEditorWarning({ unsupportedEditor }) {
	...
	return (
		<div className="legacy-editor-warning">
			<span className="icon info"></span>
			<span>
				{`Built for older XCoder versions powered by ${oldEditor}. Install with caution; some features may behave unexpectedly in the current CodeMirror version.`}
			</span>
		</div>
	);
}
...
async function showReviews({ pluginId, author, stats, onStatsChange }) {
	...
	async function initialize() {
		...
		const reviewsResult = await Promise.allSettled([loadReviews()]).then(
			(results) => results[0],
		);
		const userResult = { status: "rejected", reason: new Error("offline") };
	...
	async function loadReviews() {
		// XCoder: reviews require a server and were disabled.
		return [];
	}
	...
	async function requestJson(url, options) {
		// XCoder: review submissions require a server and were disabled.
		const response = { ok: false, status: 0, json: async () => ({}) };
		if (true) {
