...
export default function () {
	...
	const list = new Ref();
	let cmPreview = null;
	const previewDoc = `// XCoder is awesome!\nconst message = "Welcome to XCoder";\nconsole.log(message);`;
		...
		const container = (
			...
			>
				...
				<div className="terminal-line terminal-prompt">
					...
					<span style={`color:${theme.blue};`}>xcoder</span>
					<span style={`color:${theme.foreground};`}>:~$ </span>
					<span>echo "Hello, XCoder!"</span>
				</div>
				<div className="terminal-line terminal-output">Hello, XCoder!</div>
				<div className="terminal-line terminal-prompt">
					<span style={`color:${theme.green};`}>user</span>
					<span style={`color:${theme.foreground};`}>@</span>
					<span style={`color:${theme.blue};`}>xcoder</span>
	...
	function setEditorTheme({ caption, theme }) {
		...
		if (!ok) {
			alert(
				"Invalid theme",
				"This editor theme is not compatible with XCoder's CodeMirror runtime.",
			);
			return;
		}
