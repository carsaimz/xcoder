...
import { getHostHomeDir, resolveInitialDirectory } from "lib/sshSessions";
import helpers from "utils/helpers";
import Url from "utils/Url";
import TerminalComponent from "./terminal";
import TerminalTouchSelection from "./terminalTouchSelection";

const TERMINAL_SESSION_STORAGE_KEY = "xcoderTerminalSessions";
...
class TerminalManager {
	...
	async createTerminal(options = {}) {
		try {
			...
			let terminalStyle = document.getElementById("xcoder-terminal-styles");
			if (!terminalStyle) {
				terminalStyle = tag("style", {
					id: "xcoder-terminal-styles",
	...
	async setupTerminalHandlers(
		...
		// Handle xcoder CLI open commands (OSC 7777)
		terminalComponent.onOscOpen = async (type, path) => {
			// OSC 7777 is an XCoder-local CLI protocol. Remote hosts must not use it
			...
			try {
				if (type === "folder") {
					...
					toast(
						(
							strings["terminal opened folder"] || "Pasta aberta: {name}"
						).replace("{name}", name),
					);
				} else {
					// Open file in editor
				...
				toast(
					(strings["terminal open failed"] || "Falha ao abrir: {path}").replace(
						"{path}",
						path,
					),
				);
	...
	async createRemoteTerminal(storage, options = {}) {
		...
		// SSH sessions v2: a host-configured home directory wins over the
		// URL path (the storage URL usually points at "/").
		const initialDirectory = resolveInitialDirectory({
			initialDirectory: options.initialDirectory,
			homeDir: getHostHomeDir(profileId),
			urlPath: pathname,
		});
	...
	convertProotPath(prootPath) {
		...
		const packageName = window.BuildInfo?.packageName || "com.carsaimz.xcoder";
		...
		if (prootPath.startsWith("/public")) {
			// /public -> /data/user/0/com.carsaimz.xcoder/files/public
